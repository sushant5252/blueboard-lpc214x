 ...
 * Return value:
*/
esint8 file_fopen(File* file,FileSystem *fs,esint8* filename,euint8 mode)
{
    FileLocation loc;
    FileRecord wtmp;
    ...
    dir_getFatFileName((eint8 *)filename,fatfilename);
    ...
    switch(mode)
	{
        case MODE_READ:
            ...
            break;
        case MODE_WRITE:
            if(fs_findFile(fs,(eint8 *)filename,&loc,&sec)) /* File may NOT exist, but parent HAS to exist */
			{
                return(-2);
			}
            ...
            if(fs_findFreeFile(fs,(eint8 *)filename,&loc,0))
			{
            ...
            break;
        case MODE_APPEND:
			if(fs_findFile(fs,(eint8 *)filename,&loc,0)==1) /* File exists */
			{
				dir_getFileStructure(fs,&(file->DirEntry), &loc);
				file_initFile(file,fs,&loc);
