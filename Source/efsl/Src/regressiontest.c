...
int _main(int argc, char** argv)
{
	...
	switch(argv[1][0]){
		case '0':
			...
			fr = malloc(sizeof(*fr));
			if ( (file_fopen(fr,fs,(esint8*)"file.r",MODE_READ)) != 0){
		...
		case '2':
			...
			if((file_fopen(fr,fs,(esint8*)"file.r",'r'))!=0){
				printf("Unable to open %s for reading...\n","file.r");
				exit(0);
			}
			if((file_fopen(fw,fs,(esint8*)"file.w",'w'))!=0){
			...
			if((file_fopen(fr,fs,(esint8*)"file.w",'r'))!=0){
				printf("Unable to open %s for reading...\n","file.w");
				exit(0);
			}
